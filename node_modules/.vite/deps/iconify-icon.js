//#region node_modules/iconify-icon/dist/iconify-icon.mjs
/**
* (c) Iconify
*
...
function getCustomisations(node) {
	const customisations = { ...defaultCustomisations };
...
function renderIcon(parent, state) {
	...
	switch (mode) {
		case "svg":
			...
			break;
		default: node = renderSPAN(renderData, {
			...defaultIconProps,
			...iconData
		}, mode === "mask");
	}
	...
	if (oldNode) {
		if (node.tagName === "SPAN" && oldNode.tagName === node.tagName) oldNode.setAttribute("style", node.getAttribute("style"));
	...
	const IconifyIcon = class extends ParentClass {
		...
		*/
		constructor() {
			...
			this._state = setPendingState({ value: "" }, inline);
			this._queueCheck();
		}
		/**
		* Connected to DOM
		...
		*/
		attributeChangedCallback(name) {
			switch (name) {
				...
				case "noobserver":
					if (this.hasAttribute("noobserver")) this.startObserver();
		...
		*/
		_iconChanged(newValue) {
			const icon = parseIconValue(newValue, (value, name, data) => {
				...
				if (icon.data) this._gotIconData(icon);
				else state.icon = icon;
			});
			if (icon.data) this._gotIconData(icon);
			else this._state = setPendingState(icon, this._state.inline, this._state);
		}
		/**
...
* Export functions
*/
var { enableCache, disableCache, iconLoaded, iconExists, getIcon, listIcons, addIcon, addCollection, calculateSize, buildIcon, iconToHTML, svgToURL, loadIcons, loadIcon, setCustomIconLoader, setCustomIconsLoader, addAPIProvider, _api } = IconifyIconComponent;
//#endregion
